...
Agenda.prototype.define = function(name, options, processor) {
    ...
    var props = {
        type: 'definition',
        name: name,
        data: funcster.deepSerialize(this._definitions[name])
    };

    this._db.update({ name: name }, props, { upsert: true }, function(err, result) {
...
Agenda.prototype.start = function() {
    var self = this;
    if (!self._definitions.length) {
        self._db.findOne({type: 'definition'}, {}, function (err, res) {
            if (err) {
                throw(err);
                return;
            }
            ...
            if (Array.isArray(res)) {
                ...
                for (var defn in res) {
                    ...
                    var data = funcster.deepDeserialize(res[defn].data);

                    self._definitions[res[defn].name] = {
                        fn: data.fn,
                        concurrency: data.concurrency || self._defaultConcurrency,
                        priority: data.priority || 0,
                        lockLifetime: data.lockLifetime || self._defaultLockLifetime,
                        running: 0
                    };
                }
            } else {
                ...
                if (res) {
                    var data = funcster.deepDeserialize(res.data);

                    self._definitions[res.name] = {
                        fn: data.fn,
                        concurrency: data.concurrency || self._defaultConcurrency,
                        priority: data.priority || 0,
                        lockLifetime: data.lockLifetime || self._defaultLockLifetime,
    ...
    function start() {
        if (!self._processInterval) {
            self._processInterval = setInterval(processJobs.bind(self), self._processEvery);
            process.nextTick(processJobs.bind(self));
        }
    }
};
